...
date: "Reading for Lab on Monday Oct.\ 22"
...
target_ref_year <- 2005
early_target_year <- 2030
late_target_year  <- 2050

early_target_reduction <- 0.36
late_target_reduction  <- 0.80
...
goal is for emissions in `r late_target_year` to be 
`r round(100 * late_target_reduction)`% less than they were in 
`r target_ref_year`, you would 
put 
`r late_target_year` for the target year, 
`r round(100 * late_target_reduction)`% for the emissions reduction, 
and `r target_ref_year` for the reference year.  
...
so if you know the values for _P_, _g_, _e_, and _f_ in `r start_year`, 
...
$$
\begin{aligned}
  P(y) &= P(`r start_year`) \times \exp(r_P (y - `r start_year`)),\\
  g(y) &= g(`r start_year`) \times \exp(r_g (y - `r start_year`)),\\
  e(y) &= e(`r start_year`) \times \exp(r_e (y - `r start_year`)),\\
  \lefteqn{\text{and}}\\
  f(y) &= f(`r start_year`) \times \exp(r_f (y - `r start_year`)),\\
\end{aligned}
$$
...
\begin{aligned}
  \mathrm{GDP}(y) &= P(y)\times g(y)\\
    &= P(`r start_year`) \times \exp(r_p (y - `r start_year`)) 
    \times g(`r start_year`) \times \exp(r_g (y - `r start_year`))\\
    &= P(`r start_year`)\times g(`r start_year`)
    \times \exp((r_P + r_g) (y - `r start_year`))\\
    \lefteqn{\text{so}}\\
...
extrapolations of the population and per-capita GDP through `r late_target_year`. 
...
1. Open the web app at <https://ees3310.jgilligan.org/decarbonization>, 
   ...
   and write down the most current (`r start_year`) values for 
...
3. Next, calculate the rates of change of 
   ...
   year through `r start_year`, using the `lm` function in R.
...
4. Using the rates of change that you determined in part 3, 
   ...
   values for 
   _P_, _g_, _e_, and _f_ 
   in the year `r late_target_year`.
    ...
    `r late_target_year`, assuming that emissions continue to grow at 
    historical rates. 
...
5. Calculate the emissions target for each country: Set the reference year for 
   emissions reduction to `r target_ref_year`, and set the target emissions
   reduction using the table below:
    ...
    Set the target year in the web app to `r late_target_year`;
    set the reference year to `r target_ref_year`;
    ...
    `r late_target_year` in order to meet your policy goal? 
...
7. Look up what the CO~2~ emission is in `r target_ref_year` and calculate the 
   rate of change in _F_ that would be necessary to achieve your policy target. 
   For the `r late_target_year`\ calculation:
   
    a. Calculate the ratio of $F_{`r late_target_year`}/F_{`r target_ref_year`}$.
    ...
    c. Divide the logarithm by the number of years 
       ($`r late_target_year` - `r target_ref_year`$). 
    ...
    F_2017_middle_east = get_kaya_data("Middle East") %>% 
      filter(year == 2017) %$% F
    r_F_middle_east = log(F_goal_middle_east / F_2017_middle_east) / (2050 - 2017)
    r_F_middle_east
    ```
    so total emissions for the Middle East would need to drop by 
    `r round(100 * r_F_middle_east, 2)`% per year between `r start_year`
    and `r late_target_year`
...
9. How does the implied rate of decarbonization for each nation compare to the 
